		...
		semen
			...
			reaction_turf(var/turf/simulated/T, var/volume)
				...
				for(var/datum/disease/D in data["viruses"])
					...
					sex_prop.viruses += newVirus
					newVirus.holder = sex_prop
			...
			on_merge(var/list/new_data)
				if(data&&new_data)
					for(var/I in new_data)
						if(data[I]==null)
							data[I]=new_data[I]
					if(data["viruses"] || new_data["viruses"])
		...
		cockchem
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(istype(M,/mob/living/carbon)&&M.has_dna())
					...
					if(cnt_digested==20&&bonersize==COCK_NONE)
						cnt_digested=0
						humz.dna.cock["has"]=COCK_NORMAL
						M<<"You grow a cock."
						M.set_cock_block()
						humz.updateappearance()
					if(cnt_digested==10&&bonersize==COCK_NORMAL)
						cnt_digested=0
						humz.dna.cock["has"]=COCK_HYPER
						M<<"Your cock is now huge."
						M.set_cock_block()
						humz.updateappearance()
					if(bonersize!=COCK_NORMAL&&bonersize!=COCK_NONE)
		...
		decockchem
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(istype(M,/mob/living/carbon)&&M.has_dna()&&!holder.has_reagent("cockchem"))
					var/mob/living/carbon/humz=M
					var/bonersize=humz.dna.cock["has"]
					if(cnt_digested==20&&bonersize==COCK_NORMAL)
						cnt_digested=0
						humz.dna.cock["has"]=COCK_NONE
						M<<"You no longer have a cock."
						M.set_cock_block()
						humz.updateappearance()
					if(cnt_digested==10&&bonersize==COCK_HYPER)
						cnt_digested=0
						humz.dna.cock["has"]=COCK_NORMAL
						M<<"Your cock shrinks."
						M.set_cock_block()
						humz.updateappearance()
					if(cnt_digested==10&&bonersize==COCK_DOUBLE)
						cnt_digested=0
						humz.dna.cock["has"]=COCK_NORMAL
						M<<"You no longer have two cocks."
						M.set_cock_block()
						humz.updateappearance()
					if(bonersize!=COCK_NORMAL&&bonersize!=COCK_HYPER&&bonersize!=COCK_DOUBLE)
						cnt_digested=min(1,cnt_digested)
				else
					cnt_digested=min(1,cnt_digested)
				holder.remove_reagent(src.id, 1)
					...
					switch(data["count"])
						...
						if (10 to INFINITY)
							...
							if(M.vore_stomach_datum.contents.len)
								M.vore_stomach_datum.release(VORE_FLAVOUR_SILENT)
							if(M.vore_tail_datum.contents.len)
								M.vore_tail_datum.release(VORE_FLAVOUR_HURL)
							if(M.vore_breast_datum.contents.len) //ADD THIS TO LACTATE LATER
								M.vore_breast_datum.release(VORE_FLAVOUR_SILENT)
							data["count"]=0
						...
						if (30 to INFINITY)
							...
							if(prob(3))
								if(M.vore_cock_datum.check_exist()&&M.vore_womb_datum.check_exist())
									if(prob(50)) M.vore_cock_datum.release(VORE_FLAVOUR_HURL)
									else M.vore_womb_datum.release(VORE_FLAVOUR_HURL)
								else if(M.vore_cock_datum.check_exist())
									M.vore_cock_datum.release(VORE_FLAVOUR_HURL)
								else if(M.vore_womb_datum.check_exist())
									M.vore_womb_datum.release(VORE_FLAVOUR_HURL)
					data["count"]++
				holder.remove_reagent(src.id, 0.5)
				//..()
				return
