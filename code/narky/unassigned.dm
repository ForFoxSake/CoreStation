...
/datum/vore_organ
	...
	proc/digest()
			...
			*/
			if(istype(M, /mob/living))
				if(M.stat == 2 && (M.getFireLoss()>=100||!istype(M,/mob/living/carbon/human)) && digestion_factor)
					...
					flavour_text(VORE_FLAVOUR_DIGEST,M)
					M.ghostize()
					M.vore_contents_drop(src)
					M.death(1)
					owner.vore_contents.Remove(M)
					contents.Remove(M)
					qdel(M)
					digestion_count+=1
					//owner.updateappearance
					owner.do_update_body()
				...
				if(SSair.times_fired%3==1)
					...
					if(M.vore_transfer_index>=100)
						flavour_text(VORE_FLAVOUR_TRANSFER,M)
						//contents.Remove(M)
		...
		return 1
	proc/release(var/style=VORE_FLAVOUR_RELEASE,var/mob/living/prey=null,var/extra_info=0)
		if(owner.stat!=DEAD&&owner.reagents&&owner.reagents.has_reagent("novomitchem"))return
		if(style==VORE_FLAVOUR_RELEASE&&(last_release>SSair.times_fired-20||owner.stat==2))return
	...
	proc/flavour_text(var/source, var/mob/living/prey, var/extra_info=0)
		...
		else if(source==VORE_FLAVOUR_RELEASE)
			var/live_people=0
			for(var/mob/living/M in contents)
				live_people=1
				break
		...
		else if(source==VORE_FLAVOUR_HURL)
			var/live_people=0
			for(var/mob/living/M in contents)
				live_people=1
				break
	...
	proc/relaymove(var/mob/user, var/direction)
		...
		if(prey.a_intent=="help"&&!exterior)
			...
			if(exterior||escape||owner.stat==2)
				release(VORE_FLAVOUR_ESCAPE,prey)
			else
				integrity-=15
				...
				if(integrity<=0)
					release(VORE_FLAVOUR_HURL,prey)
					integrity=0
					owner.Stun(rand(2,4))
	...
	proc/transfer_to_other(var/datum/vore_organ/targ, var/mob/living/prey=null)
		if(prey)
			if(targ.owner!=owner)
				owner.vore_contents.Remove(prey)
			organ_leave(prey)
	...
	flavour_text(var/source, var/mob/living/prey, var/extra_info=0)
		if(source==VORE_FLAVOUR_DIGEST)
			...
			if(istype(owner.loc,/turf))
				sex_splatter(owner.loc, owner, milk_type)
			var/live_people=0
			for(var/mob/living/M in contents)
				live_people=1
				break
			...
			owner.vore_balls_datum.release(VORE_FLAVOUR_SILENT,prey)
		else if(source==VORE_FLAVOUR_HURL)
			digestion_count+=owner.vore_balls_datum.digestion_count
			remembered_bans|=owner.vore_balls_datum.remembered_bans
			remains += owner.vore_balls_datum.remains
		...
		else if(source==VORE_FLAVOUR_TRANSFER)
...
/datum/vore_organ/balls
	...
	assoc_loc="groin"
	assoc_fluid="semen"
	release(var/style=VORE_FLAVOUR_RELEASE,var/prey=null)
		if(style==VORE_FLAVOUR_HURL)
	...
	digest()
		...
		if(owner.get_shoes())
			...
			for(var/obj/item/O in contents)
				...
				return
	flavour_text(var/source, var/mob/living/prey, var/extra_info=0)
		if(source==VORE_FLAVOUR_RELEASE)
...
/mob/living/proc/vore_contents_drop(var/datum/vore_organ/conta=null)
	...
	if(!conta)return
	for(var/mob/living/M in src.vore_contents)
		src.vore_contents.Remove(M)
	...
	for(var/obj/item/W in src)
		...
		if(conta)
			...
			conta.owner.vore_contents.Add(W)
...
/obj/item/clothing/under/diaper
	name = "space diaper"
	desc = "You'd think this would be underwear."
	icon_state = "diaper"
	item_state = "diaper"
	item_color = "diaper"
...
/mob/living/proc/underwear_toggle()
	set name = "Force Underwear Update"
	set category = "Vore"
	if(istype(src,/mob/living/carbon/human))
		var/mob/living/carbon/human/humz=src
